...
#define RESULTS_LISTBOX_DEBUGMSG 2

//// end generated include

#define CODECOMP_SCROLL_UP      1
#define CODECOMP_SCROLL_DOWN    2

wxMutex notebook_mutex;
bool show_codeComp = false;
int codeComp_scroll = 0;
bool codeComp_tabComplete = false;
int codeComp_startPos = 0;
int codeComp_currentPos = 0;
bool codeComp_mouseClick = false;
bool codeComp_lockIn = false;
bool codeComp_comma = false;
wxString codeComp_current_symbol = _("");
wxString codeHint_current_symbol = _("");
int codeHint_current_symbol_index = -1;
int codeHint_current_user_index = -1;
bool codeComp_isUDT = false;
int codeComp_udt_index = -1;
...
{
    private:
        //rcbasic_edit_codeCompletion_window* codeComp;
        //bool show_codeComp;
    public:

        rc_styledTextCtrl(wxWindow* parent_nb, wxString src_name);
        ~rc_styledTextCtrl();

        void setShowComp(bool flag);
        bool codeCompIsShown();
...
{
    private:
        rc_styledTextCtrl* txtCtrl;
        wxFileName sourcePath;
        bool text_changed;
        wxString temp_text;
        rcbasic_edit_codeCompletion_window* codeComp;
    public:
        rcbasic_edit_txtCtrl(wxFileName src_path, wxAuiNotebook* parent_nb, rcbasic_edit_codeCompletion_window* codeComp);
        ~rcbasic_edit_txtCtrl();
        rc_styledTextCtrl* getTextCtrl();
        wxFileName getSourcePath();
        void setTextCtrl(rc_styledTextCtrl* t_ctrl);
...
struct rcbasic_parse_symbol_search
{
    wxString symbol_name;
    int project_symbol_index;
    wxArrayInt comp_index;
};

struct rcbasic_parse_udt_item
{
    wxArrayString symbol_name;
    int scope;
};
...
class rcbasic_edit_frame : public rc_ideFrame
{
    private:
        ...
        rcbasic_edit_codeCompletion_window* codeComp;
        rcbasic_edit_codeHint_window * codeHint;
        rc_codeComp_symbolDatabase codeComp_symbol_db;
        rc_codeComp_symbolDatabase codeComp_user_db;
        rc_udt_db codeComp_udt_db;
        ...
        rc_styledTextCtrl* activated_project_item_page;
	...
	protected:
		...
		void onNotebookPageIsChanging( wxAuiNotebookEvent& event );
	...
	public:
		...
		rcbasic_edit_txtCtrl* getCurrentFile()
		{
            ...
            {
                if(open_files[i]->getTextCtrl() == (rc_styledTextCtrl*)sourceFile_auinotebook->GetPage(selected_page))
                    return open_files[i];
            }
		...
		int searchNextPrev(rc_styledTextCtrl* t, int search_type);
		...
		void applyScheme(rc_styledTextCtrl* rc_txtCtrl);
		...
		void updateFont(rc_styledTextCtrl* t);

		rc_styledTextCtrl* pre_parsed_page;
        rc_styledTextCtrl* parsed_page;
        ...
        void showCodeComp(wxArrayString cc_list);
        void showCodeHint(wxString cc_token, int arg_num);
        wxString getTokenAtCaret(int pos, int* arg_num);
        void loadCodeCompDocs();
        int getCodeCompUDT(wxString udt_scope);
