...
bool dist_desktop(int dist_os, int arch)
{
    ...
    while(dfile.compare("") != 0)
    {
        ...
        if(dfile.compare("rcbasic4")==0 || dfile.compare("rcbasic4_win")==0 || dfile.compare("rcbasic")==0)
            dfile_dst = appendFileToPath(DIST_OUTPUT_DIR, PROJECT_OUTDIR_NAME);
        else if(dfile.compare("rcbasic4.exe")==0 || dfile.compare("rcbasic4_win.exe")==0 || dfile.compare("rcbasic.exe")==0)
